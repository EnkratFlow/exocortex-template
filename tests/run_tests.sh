...
if PYTHONDONTWRITEBYTECODE=1 python3 "$TEMPLATE_DIR/.exocortex/scripts/generate_command_adapters.py" --check >/dev/null; then
    ok "canonical 26-command registry generates exactly 78 current adapters"
else
    bad "canonical 26-command registry generates exactly 78 current adapters"
fi
...
if [ "$(find "$target/.agents/skills" -type f -name SKILL.md | wc -l | tr -d ' ')" = "26" ] \
    && [ "$(find "$target/.claude/skills" -type f -name SKILL.md | wc -l | tr -d ' ')" = "26" ] \
    && [ "$(find "$target/.cursor/skills" -type f -name SKILL.md -exec grep -lF 'GENERATED BY .exocortex/scripts/generate_command_adapters.py' {} + | wc -l | tr -d ' ')" = "26" ] \
