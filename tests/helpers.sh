...
# Runs install.sh against the current working tree of the template.
# Creates a temporary git repo from the working tree so that staged-but-not-yet-committed
...
run_install() {
    ...
    # Uses git archive (fast, shallow) rather than copying the full .git/ object store.
    local snap
    snap=$(mktemp -d)
    # Export committed state
    git -C "$template" archive HEAD | tar -x -C "$snap" 2>/dev/null
    # Overlay any staged changes (files added/modified in the index)
    git -C "$template" diff --cached --name-only 2>/dev/null | while IFS= read -r f; do
        [ -f "$template/$f" ] && mkdir -p "$snap/$(dirname "$f")" && \
            cp "$template/$f" "$snap/$f"
    done
    # Also overlay deleted staged files
    ...
    git -C "$snap" -c user.email="test@exocortex" -c user.name="exocortex-test" \
        add -A >/dev/null 2>&1
    git -C "$snap" -c user.email="test@exocortex" -c user.name="exocortex-test" \
        commit -q -m "test snapshot" >/dev/null 2>&1
