...
MANIFEST_TMP="$TMPDIR/manifest-new"
touch "$MANIFEST_TMP"
...
git clone --quiet --depth 1 --branch "$BRANCH" "$REPO_URL" "$TMPDIR/exocortex-template" 2>&1 | grep -v "^$" || true
echo "  ✓ Downloaded"
...
safe_copy_dir "$TMPDIR/exocortex-template/.exocortex" ".exocortex" ".exocortex/"

# Copy editor pointer files (thin pointers to AI_BOOTSTRAP.md)
for pfile in CLAUDE.md .windsurfrules; do
...
safe_copy_dir "$TMPDIR/exocortex-template/.cursor/commands" ".cursor/commands" "commands"
safe_copy_dir "$TMPDIR/exocortex-template/.cursor/skills"   ".cursor/skills"   "skills"
safe_copy_dir "$TMPDIR/exocortex-template/.cursor/rules"    ".cursor/rules"    "rules"
safe_copy_dir "$TMPDIR/exocortex-template/.cursor/agents"   ".cursor/agents"   "agents"

# ── VS Code Copilot role skills (.github/skills/) ─────────────────────

echo ""
echo "🤖 Installing VS Code Copilot skills..."
safe_copy_dir "$TMPDIR/exocortex-template/.github/skills" ".github/skills" ".github/skills"

# ── Claude / VS Code workflow commands (.claude/skills/) ─────────────

echo ""
echo "🧠 Installing workflow command skills..."
safe_copy_dir "$TMPDIR/exocortex-template/.claude/skills" ".claude/skills" ".claude/skills"
