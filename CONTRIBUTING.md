...
- pinned-source and complete-checksum validation;
- clean install, repeated install, migration, collision preservation, and
  guarded existing-repository update;
- protected project data, symlink/path safety, fault injection, rollback, and
  idempotency;
...
- Preserve the documented runtime baseline: Bash 3.2+, Python 3.9+ with Unix
  `fcntl` for guarded apply, Git, `shasum`, `awk`, `tar`, `find`, `grep`, `sed`,
