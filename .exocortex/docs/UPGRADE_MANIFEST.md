...
`.exocortex/SESSION_CONTEXT_BACKUP_*.md` are protected whether untracked or
already tracked by Git. The classification does not broadly cover arbitrary
backup files. Their ignore rules prevent future accidental tracking; it cannot remove an existing Git index entry.
The updater preserves already-tracked sidecars and reports them, while any index or history cleanup is a separate
