...
echo "  2. Copy integration config to .cursorrules (or your AI assistant)"
...
ln -sf .exocortex/examples/.cursorrules .cursorrules
...
## Command System
When user types a command starting with "/", check if it exists in .exocortex/commands/

If found, execute the steps from the JSON spec in order:
1. For "shell" type steps: run the command and capture output
2. For "ai" type steps: execute the AI instruction using available context
3. Auto-verify success messages and provide command completion status

Commands available:
- /work: Load context for work session
- /drill <topic>: Deep dive memory search
- /shortterm: 7-31 day memory analysis  
- /longterm: 31+ day historical analysis
- /subconscious: Cross-cutting pattern detection

## Context Loading
Always read .exocortex/SESSION_CONTEXT.md when available for recent memory.
This file is automatically generated and contains:
- Recent activity summary (0-7 days)
- Thematic patterns (7-31 days)
- Current work state and git context

## Memory System Integration
The exocortex provides four memory tiers:
- RIGHT NOW (0-7 days): Specific recent events and actions
- SHORT-TERM (7-31 days): Themes and patterns in recent work
- LONG-TERM (31+ days): Historical arcs and major developments  
- SUBCONSCIOUS (all events): Cross-cutting patterns and blind spots

Use memory context to provide informed assistance with:
- Code reviews based on project history
- Architecture decisions informed by past choices
- Problem-solving using previous solution patterns
- Planning based on historical development cycles

## Event Creation
When significant work is completed, suggest creating an event:
"Consider documenting this work: .exocortex/scripts/create_event.sh 'Brief description'"
