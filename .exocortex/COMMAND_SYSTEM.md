...
| `/check-keys` | Validate all API keys — status, purpose, location | `check_keys.sh` → `check_keys.py` |
...
- `post_to_hub.sh` — Post to ecosystem hub (configure EXOCORTEX_HUB_DIR in ~/.exocortex/.env)
...
│   └── check-keys.json         # API key validation (safe — no values exposed)
...
**Cursor:** Add exocortex user rule in Settings > General > Rules for AI (one-time setup, see README.md for the rule text). The AI reads `.exocortex/AI_BOOTSTRAP.md` when commands are invoked.
**VS Code / other editors:** First message: "read .exocortex/AI_BOOTSTRAP.md"
**Then:** All commands work identically in any editor.
**Note:** The AI may not always follow rules automatically. If it doesn't recognize a command, prompt: "read .exocortex/AI_BOOTSTRAP.md"
