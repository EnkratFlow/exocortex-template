...
When the user types `/name` or `name`:
...
5. Treat every requested mutation as a proposal until a current one-time
   capability is accepted by the registered guarded executor.
6. Treat every outward action as a second, destination-specific gate. Never
   infer it from a local write, save, handoff, lifecycle transition, or prior
   approval.
...
- `user_choice`: wait for the user. A choice may request the next gate, but it
  does not bypass capability validation or combine downstream gates.
...
3. A human reviews the immutable descriptor and separately approves the exact
   destination, method, outward effect, expiry, and executor.
