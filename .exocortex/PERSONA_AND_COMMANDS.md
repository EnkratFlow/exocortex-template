...
### `/check-keys` - Validate API Keys

**When to use:** After rotating keys, adding a new provider, or any time you want to confirm keys are working
**What it does:** Reads `.exocortex/key-registry.json`, tests each key via live API call, shows status + purpose + location. Shows last 4 chars only — never exposes key values.
**Security:** AI must run `bash .exocortex/scripts/check_keys.sh` only. Never read, grep, or display key values.
**Add new provider:** Edit `.exocortex/key-registry.json` — no script changes needed
