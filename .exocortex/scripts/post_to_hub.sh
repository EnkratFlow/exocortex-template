#!/bin/bash
# Post event summary to the ecosystem hub
...
EVENT_FILE="$1"
HUB_DIR="${EXOCORTEX_HUB_DIR:-}"
if [ -z "$HUB_DIR" ]; then
    # Try to find a hub by looking for a sibling repo with a hub directory.
    # Set EXOCORTEX_HUB_DIR in ~/.exocortex/.env to configure this explicitly.
    exit 0
fi
