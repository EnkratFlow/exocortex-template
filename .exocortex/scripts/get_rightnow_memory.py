...
STRUCTURE — each session gets a bold date/machine anchor line, then 2-3 SHORT paragraphs:

**[Date] · [machine] · [editor] · [time of day if known]**
[2-3 sentences about what happened in this session. Concrete specifics — numbers, file names, what got done.]

[1-2 sentences about what's still open or what you were thinking about. Future intentions belong here.]

If there are multiple sessions in the last 7 days, show each one with its own anchor line. Most recent first. Keep each session block SHORT — the whole output should be scannable in 10-15 seconds.
...
WHAT TO AVOID:
- NO bullet lists, NO category headers, NO emoji-labeled sections
...
pass1_result = call_ai([{"role": "user", "content": prompt}], max_tokens=1500)
...
- NO bullet lists, NO emoji-labeled sections, NO category headers
