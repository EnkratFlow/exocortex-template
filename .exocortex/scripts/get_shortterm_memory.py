...
if not events:
    # Even with no saved events, check if git has commits in this window
    # before giving up — commits are the authoritative record of what was built
    try:
        repo_root = exocortex_dir.parent
        since_str = thirty_one_days_ago.strftime('%Y-%m-%d')
        until_str = seven_days_ago.strftime('%Y-%m-%d')
        probe = subprocess.run(
            ["git", "log", f"--since={since_str} 00:00", f"--until={until_str} 23:59",
             "--format=%h", "--max-count=1"],
            cwd=repo_root, capture_output=True, text=True, timeout=5
        )
        if not probe.stdout.strip():
            print("📅 SHORT-TERM (7-31 Days Ago)\n\nNo events and no commits in this period.")
            sys.exit(0)
        # There are commits — continue with empty events list; git supplement will fill in
    except Exception:
...
period_start = events[-1]['date'].strftime('%b %d') if events else thirty_one_days_ago.strftime('%b %d')
period_end = events[0]['date'].strftime('%b %d') if events else seven_days_ago.strftime('%b %d')
...
# Supplement with git log for 7-31 day window — fills gaps on days without saved events
git_supplement = ""
try:
    repo_root = exocortex_dir.parent
    since_str = thirty_one_days_ago.strftime('%Y-%m-%d')
    ...
    if result.returncode == 0 and result.stdout.strip():
        ...
        git_supplement = (
            f"\n\n━━━ GIT LOG SUPPLEMENT (7-31 days ago) ━━━\n"
            f"NOTE: Commits on dates with no event file are marked [NO EVENT — git only].\n"
            f"Use commit bodies as the authoritative record of what was built on those days.\n\n"
        )
        current_date = None
    ...
    pass  # git supplement is optional
