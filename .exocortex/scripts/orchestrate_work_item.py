...
def route_models(task_path: Path, catalog_path: Path) -> Dict[str, Any]:
    task = load_json(task_path)
    catalog = load_json(catalog_path)
    require_exact_keys(task, ["required_capabilities", "risk", "delegates"], [], "routing task")
    require_exact_keys(catalog, ["models"], [], "routing catalog")
    if task["risk"] not in {"low", "medium", "high", "critical"}:
        raise ProtocolError("invalid_risk", "routing risk is invalid")
    if not isinstance(task["required_capabilities"], list) or any(
        not isinstance(value, str) or not value for value in task["required_capabilities"]
    ):
        raise ProtocolError("invalid_capabilities", "required_capabilities must contain non-empty strings")
    ...
    if not isinstance(catalog["models"], list):
        raise ProtocolError("invalid_catalog", "models must be an array")
    risk_rank = {"low": 0, "medium": 1, "high": 2, "critical": 3}
    candidates = []
    for model in catalog["models"]:
        if not isinstance(model, dict):
            raise ProtocolError("invalid_model", "model catalog entries must be objects")
        require_exact_keys(
            model,
            ["id", "available", "parent_capable", "capabilities", "max_risk", "cost_rank"],
            [],
            "model catalog entry",
        )
        if model["max_risk"] not in risk_rank:
            raise ProtocolError("invalid_model_risk", "model max_risk is invalid")
        if not isinstance(model["available"], bool) or not isinstance(model["parent_capable"], bool):
            raise ProtocolError("invalid_model_flags", "model availability flags must be booleans")
        if not isinstance(model["capabilities"], list) or any(
            not isinstance(value, str) or not value for value in model["capabilities"]
        ):
            raise ProtocolError("invalid_model_capabilities", "model capabilities must contain non-empty strings")
        if not isinstance(model["cost_rank"], (int, float)) or isinstance(model["cost_rank"], bool):
            raise ProtocolError("invalid_cost_rank", "model cost_rank must be numeric")
        if not model["available"] or not model["parent_capable"]:
            continue
        if risk_rank[model["max_risk"]] < risk_rank[task["risk"]]:
            continue
        if not set(task["required_capabilities"]).issubset(set(model["capabilities"])):
            continue
        candidates.append(model)
    if not candidates:
        raise ProtocolError("no_capable_parent", "no available model can own the bounded task")
    parent = min(candidates, key=lambda item: (item["cost_rank"], item["id"]))

    delegates = []
    for bounded in task["delegates"]:
        if not isinstance(bounded, dict):
            raise ProtocolError("invalid_delegate", "delegate entries must be objects")
        require_exact_keys(bounded, ["id", "required_capabilities", "risk"], [], "delegate task")
        require_id(bounded["id"], "delegate id")
        if bounded["risk"] not in risk_rank:
            raise ProtocolError("invalid_delegate_risk", "delegate risk is invalid")
        if not isinstance(bounded["required_capabilities"], list) or any(
            not isinstance(value, str) or not value for value in bounded["required_capabilities"]
        ):
            raise ProtocolError("invalid_delegate_capabilities", "delegate capabilities must contain non-empty strings")
        eligible = [
            model
            for model in catalog["models"]
            if model["available"]
            and risk_rank[model["max_risk"]] >= risk_rank[bounded["risk"]]
            and set(bounded["required_capabilities"]).issubset(set(model["capabilities"]))
        ]
        if not eligible:
            delegates.append({"task_id": bounded["id"], "model_id": parent["id"], "escalated_to_parent": True})
        else:
            chosen = min(eligible, key=lambda item: (item["cost_rank"], item["id"]))
            delegates.append({"task_id": bounded["id"], "model_id": chosen["id"], "escalated_to_parent": False})
    return {
        "ok": True,
        "policy": "least_expensive_capable_parent_then_delegate",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    route_parser.add_argument("--catalog", required=True, help="project-relative model catalog JSON")
