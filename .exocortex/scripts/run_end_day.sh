...
if git rev-parse --git-dir >/dev/null 2>&1; then
    ...
    # Today's commits — full message bodies (subject + body, not just subject)
    FIRST_COMMIT_TODAY=$(git log --since="$TODAY 00:00" --reverse --format="%H" 2>/dev/null | head -1)
    TODAY_COMMIT_SUBJECTS=$(git log --since="$TODAY 00:00" --format="%h %s" 2>/dev/null || true)
    if [ -n "$TODAY_COMMIT_SUBJECTS" ]; then
        COMMIT_COUNT=$(echo "$TODAY_COMMIT_SUBJECTS" | wc -l | tr -d ' ')
        echo "Commits today ($COMMIT_COUNT):"
        echo "$TODAY_COMMIT_SUBJECTS" | sed 's/^/  /'
        echo ""
        echo "Full commit messages (subject + body — this is the authoritative record of what was built):"
        echo ""
        git log --since="$TODAY 00:00" --format="commit %h — %s%n%b%n---" 2>/dev/null | sed 's/^/  /' || true
    else
    ...
    # Last 7 days git log — supplementary context for days with no saved events
    echo "## GIT LOG LAST 7 DAYS (supplementary — fills gaps when /save was not run)"
    SEVEN_DAYS_AGO=$(date -v-7d '+%Y-%m-%d' 2>/dev/null || date --date='7 days ago' '+%Y-%m-%d' 2>/dev/null || echo "")
    if [ -n "$SEVEN_DAYS_AGO" ]; then
        git log --since="$SEVEN_DAYS_AGO 00:00" --format="commit %h — %ad — %s%n%b%n---" --date=short 2>/dev/null | head -300 | sed 's/^/  /' || true
    fi
    echo ""
...
# ── 5b. Claude Code Session History ───────────────
echo "## CLAUDE CODE SESSION HISTORY"
SCRIPT_HISTORY="$SCRIPT_DIR/get_claude_history.py"
if [ -f "$SCRIPT_HISTORY" ]; then
    python3 "$SCRIPT_HISTORY" 2>/dev/null || echo "  (could not read Claude Code history)"
else
    echo "  (get_claude_history.py not found — skipping)"
fi
echo ""
