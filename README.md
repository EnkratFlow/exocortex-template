...
> **Public releases are authenticity-gated.** Version 3.3.2 selects GitHub's
...
> require `gh release verify v3.3.2 -R github.com/EnkratFlow/exocortex-template`
> to pass and
...
currently have open. Use only the official GitHub release v3.3.2 from
...
replacement. A temporary clone of v3.3.2 is the update source only. An approved
...
v3.3.2 GitHub release notes. Stop if either differs. Do not substitute `main`,
...
EXOCORTEX_LOCAL_SOURCE=/tmp/exocortex-template-v3.3.2 \
EXOCORTEX_CANDIDATE_DIGEST=<sha256-computed-from-verified-release-asset> \
  bash /tmp/exocortex-template-v3.3.2/install.sh "project-name"
```
...
bash /tmp/exocortex-template-v3.3.2/scripts/safe-update.sh \
  --template /tmp/exocortex-template-v3.3.2 \
...
Version 3.3.2 selects GitHub's immutable-release attestation for
