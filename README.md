...
> **Public releases are authenticity-gated.** Version 3.3.3 selects GitHub's
...
> require `gh release verify v3.3.3 -R github.com/EnkratFlow/exocortex-template`
> to pass and
...
currently have open. Use only the official GitHub release v3.3.3 from
...
replacement. A temporary clone of v3.3.3 is the update source only. An approved
...
(
set -eu
gh release verify v3.3.3 -R github.com/EnkratFlow/exocortex-template
mkdir -m 700 /tmp/exocortex-release-verify-v3.3.3
gh release download v3.3.3 -R github.com/EnkratFlow/exocortex-template \
  --pattern SHA256SUMS --dir /tmp/exocortex-release-verify-v3.3.3
gh release verify-asset v3.3.3 \
  /tmp/exocortex-release-verify-v3.3.3/SHA256SUMS \
  -R github.com/EnkratFlow/exocortex-template
git clone --depth 1 --branch v3.3.3 \
  https://github.com/EnkratFlow/exocortex-template.git \
  /tmp/exocortex-template-v3.3.3
cmp -s /tmp/exocortex-release-verify-v3.3.3/SHA256SUMS \
  /tmp/exocortex-template-v3.3.3/SHA256SUMS
git -C /tmp/exocortex-template-v3.3.3 rev-parse HEAD
)
```
...
```bash
shasum -a 256 /tmp/exocortex-template-v3.3.3/SHA256SUMS
```

On Linux or inside WSL:

```bash
sha256sum /tmp/exocortex-template-v3.3.3/SHA256SUMS
```

Compare both outputs with the peeled commit and candidate digest in the
v3.3.3 GitHub release notes. Stop if either differs. Do not substitute `main`,
...
EXOCORTEX_LOCAL_SOURCE=/tmp/exocortex-template-v3.3.3 \
EXOCORTEX_CANDIDATE_DIGEST=<sha256-computed-from-verified-release-asset> \
  bash /tmp/exocortex-template-v3.3.3/install.sh "project-name"
```
...
bash /tmp/exocortex-template-v3.3.3/scripts/safe-update.sh \
  --template /tmp/exocortex-template-v3.3.3 \
...
Version 3.3.3 selects GitHub's immutable-release attestation for
