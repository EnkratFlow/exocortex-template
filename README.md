...
- `.exocortex/control/MODEL_ROUTING.md` selects by capability, risk,
  availability, reliability, latency, and cost—not permanent model names.
...
authority. Push, PR, merge, deployment, service action, external sync, and
template promotion are separate gates.
...
rehearse in disposable state, show the complete scope, and stop for exact
approval before each mutation. Installation never bundles commit, push, PR,
merge, deployment, synchronization, or promotion.
...
separate approval may create one clean isolated Git worktree from the exact
approved target HEAD and run the installer only there. Preserve the SHA-256 of
the reviewed `SHA256SUMS` as the separately approved candidate digest; do not
derive approval from an unreviewed source at install time:
...
Worktree creation and installation are separate local mutations requiring exact
approval. The installer must run non-interactively with a fake `HOME`. Direct
installation into a shared or primary checkout is unsupported because the
current clean installer writes in place and has no restore archive. Project
installation never grants global editor-home, launchd, provider, credential,
hub, deployment, Git publication, or external-sync authority.
...
is a separate bootstrap gate; the updater must not create it after consuming
apply authority.
...
Apply to a real repository only after the rehearsal evidence is accepted and a
separate exact update gate is approved. Re-run with the same pinned template
and candidate digest, `--apply`, the exact one-time capability, and the
registered executor identity. The updater has no interactive or implicit
approval prompt.
...
mismatched checksums fail before target mutation. Dry-run leaves the target
unchanged, writes the restore archive only beneath the explicit disposable
backup location, and uses private staging beneath `${TMPDIR:-/tmp}` (or the
platform's equivalent system temporary directory). Treat both locations as
disposable evidence storage and verify their permissions.
...
when its current bytes still match its prior install-manifest hash and its
canonical replacement installed successfully. Customized or unknown wrappers
are preserved with an `EXOCORTEX_ADAPTER_COLLISION_PRESERVED` warning.
